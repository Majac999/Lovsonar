"""LovSonar v7.5 - Strategisk Radar for Byggevarehandelen (Nøytral)"""
...
# Generelle bransje-nøkkelord (Obs BYGG er fjernet her)
KEYWORDS_SEGMENT = [
...
# Nøytral database og brukeragent
DB_PATH = "lovsonar_v7_5.db"
USER_AGENT = "LovSonar/7.5 (Strategic Intelligence Radar)"
    ...
    for pattern in patterns:
        ...
        if match:
            try:
                ...
                if m_name in MONTHS_NO:
                    return datetime(year, MONTHS_NO[m_name], day), match.group(0)
            except: continue
    return None, ""
...
def generate_html_report(rows: list) -> str:
    ...
    <p>{len(rows)} relevante treff funnet | {datetime.now().strftime('%Y-%m-%d')}</p></div>"""
    for row in rows:
...
if __name__ == "__main__":
    logger.info("🚀 LovSonar v7.5 starter...")
